...
// Add a note here that what does this module do.
...
export const unsent_messages = (function () {
    ...
    exports.is_empty = function () {
        if (messages_to_send === undefined || messages_to_send.length === 0) {
            return true;
        }
        return false;
    }
...
function start_compose_actions() {
    ...
    // It may happen that the user has opened a new tab after reconnecting
    // to the server. In this case, we want to focus of compose textarea so that
    // compose box doesn't get close on narrowing to recent topics section.
    // $("#compose-textarea").show();

    const unsent_message_template = render_compose_unsent_message();
    console.log("unsent_msg_template => ", unsent_message_template);
...
export function clear_unsent_message_warning(event) {
