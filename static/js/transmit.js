...
export function send_message(request, on_success, error) {
    ...
    // if the message is not sent to the server (because of any possible
    // error) then it will be lost during the server reload. Hence, here
    // we are storing that message content (in `compose-textarea` when
    // there is an error) so that it is not lost during the server reload.
    ...
    channel.post({
        ...
        error(xhr, error_type) {
            ...
            if (channel.is_server_unreachable(xhr) && current_unsent_msg_count < max_unsent_msg_count) {
